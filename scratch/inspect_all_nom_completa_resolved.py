...
with zipfile.ZipFile(file_path, 'r') as z:
    ...
    # Find all cells in columns M and N
    matches = re.finditer(r'<c r="([M-N][0-9]+)"([^>]*)>(.*?)</c>|<c r="([M-N][0-9]+)"([^>]*)\/>', sheet_data, re.DOTALL)
    
    print("Cells in Columns M and N:")
    for m in matches:
        ...
        print(f"Cell {ref}: Value={str_val}, Formula={formula}")
