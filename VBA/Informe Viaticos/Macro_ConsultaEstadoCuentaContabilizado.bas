'Attribute VB_Name = "ModuloConsultaEstadoCuentaContabilizado"
' Macro para consultar estado de cuenta de viaticos - Solo CONTABILIZADO desde PostgreSQL
' Base de datos: solid
' Servidor: 192.168.0.21:5432
' Usuario: postgres
' Autor: Sistema de Utilidades
' Fecha: 2026
' Nota: Variante de V2 que filtra unicamente registros con estado CONTABILIZADO
...
Sub ConsultarEstadoCuentaContabilizado(Optional v_empleado As String = "", _
                                    Optional v_nombreempleado As String = "", _
                                    Optional v_desde As String = "", _
                                    Optional v_hasta As String = "")
    ' Conecta a PostgreSQL y consulta el estado de cuenta de viaticos - Solo CONTABILIZADO
    ...
    ' Solo incluye registros con estado CONTABILIZADO
    ...
    ' Consulta SQL con parametros (dividida en partes para evitar limite de continuaciones)
    ...
    ' Parte 2: SELECT columnas basicas + valores
    sqlSelect1 = "SELECT " & vbCrLf & _
                 ...
                 "    m.radicado AS ""RADICADO""," & vbCrLf & _
                 "    CASE WHEN m.tipo = 'CONSIGNACION' THEN m.valor ELSE 0 END AS ""VALOR CONSIGNACION""," & vbCrLf & _
                 "    CASE WHEN m.tipo = 'LEGALIZACION' THEN m.valor ELSE 0 END AS ""VALOR LEGALIZACION""," & vbCrLf
    
    ' Parte 3: SUM saldo acumulado y observaciones
    Dim sqlSelect2Saldo As String
    Dim sqlSelect2Obs As String
    
    sqlSelect2Saldo = "    SUM(" & vbCrLf & _
                      "        CASE " & vbCrLf & _
    ...
    sqlSelect2Obs = "    m.observaciones AS ""OBSERVACIONES""" & vbCrLf
    
    sqlSelect2 = sqlSelect2Saldo & sqlSelect2Obs
    
    ' Parte 4: FROM con subqueries agrupados y filtro CONTABILIZADO
    Dim sqlFromConsig As String
    Dim sqlFromUnion As String
    Dim sqlFromLegal As String
    
    ' Subquery consignaciones CONTABILIZADO agrupadas por radicado
    sqlFromConsig = "FROM (" & vbCrLf & _
                    "    SELECT MIN(codigo) AS codigo," & vbCrLf & _
                    "        fechaaplicacion, empleado," & vbCrLf & _
                    "        MAX(nombreempleado) AS nombreempleado," & vbCrLf & _
                    "        codigoconsignacion AS radicado," & vbCrLf & _
                    "        SUM(valor::numeric) AS valor," & vbCrLf & _
                    "        'CONSIGNACION' AS tipo," & vbCrLf & _
                    "        MAX(observaciones) AS observaciones" & vbCrLf & _
                    "    FROM consignacion" & vbCrLf & _
                    "    WHERE UPPER(TRIM(estado)) = 'CONTABILIZADO'" & vbCrLf & _
                    "    GROUP BY fechaaplicacion, empleado, codigoconsignacion" & vbCrLf
    
    sqlFromUnion = "    UNION ALL" & vbCrLf
    
    ' Subquery legalizaciones CONTABILIZADO agrupadas por radicado
    sqlFromLegal = "    SELECT MIN(codigo) AS codigo," & vbCrLf & _
                   "        fechaaplicacion, empleado," & vbCrLf & _
                   "        MAX(nombreempleado) AS nombreempleado," & vbCrLf & _
                   "        codigolegalizacion AS radicado," & vbCrLf & _
                   "        SUM(valortotal::numeric) AS valor," & vbCrLf & _
                   "        'LEGALIZACION' AS tipo," & vbCrLf & _
                   "        MAX(observaciones) AS observaciones" & vbCrLf & _
                   "    FROM legalizacion" & vbCrLf & _
                   "    WHERE UPPER(TRIM(estado)) = 'CONTABILIZADO'" & vbCrLf & _
                   "    GROUP BY fechaaplicacion, empleado, codigolegalizacion" & vbCrLf & _
                   ") m" & vbCrLf & _
                   "CROSS JOIN params p" & vbCrLf
    
    sqlFrom = sqlFromConsig & sqlFromUnion & sqlFromLegal
    ...
    Debug.Print "=========================================="
    Debug.Print "DIAGNOSTICO DE RENDIMIENTO - ESTADO CUENTA CONTABILIZADO"
    Debug.Print "=========================================="
    ...
    Debug.Print "Ejecutando consulta de estado de cuenta CONTABILIZADO..."
    ...
    Set ws = ThisWorkbook.Worksheets("Estado_Contabilizado")
    If ws Is Nothing Then
        Set ws = ThisWorkbook.Worksheets.Add
        ws.Name = "Estado_Contabilizado"
    Else
    ...
    On Error Resume Next
    ws.ListObjects.Add(xlSrcRange, rangoTabla, , xlYes).Name = "TablaEstadoContabilizado"
    On Error GoTo ErrorHandler
    
    ' Formatear tabla de Excel
    With ws.ListObjects("TablaEstadoContabilizado")
    ...
    On Error Resume Next
    ' Columnas: A=CODIGO, B=FECHA, C=CEDULA, D=EMPLEADO, E=RADICADO,
    '           F=VALOR CONSIGNACION, G=VALOR LEGALIZACION, H=SALDO, I=OBSERVACIONES
    ...
    ws.Range("F6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoContabilizado[VALOR CONSIGNACION])"
    ws.Range("G6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoContabilizado[VALOR LEGALIZACION])"
    ws.Range("F6:G6").NumberFormat = "$#,##0;[Red]-$#,##0"
    ws.Range("F6:G6").Font.Bold = True
    ws.Range("F5").Value = "TOTALES (CONTABILIZADO)"
    ...
    mensajeExito = "Consulta completada exitosamente." & vbCrLf & vbCrLf & _
                   "Filtro: Solo estado CONTABILIZADO" & vbCrLf & _
                   ...
                   "Los resultados se encuentran en la hoja 'Estado_Contabilizado'" & vbCrLf & _
                   "Ver ventana Inmediato (Ctrl+G) para mas detalles."
    
    MsgBox mensajeExito, vbInformation, "Consulta Estado de Cuenta - Contabilizado"
...
ErrorHandler:
    ...
    Application.StatusBar = False
    Application.ScreenUpdating = True
    MsgBox mensajeError, vbCritical, "Error en Consulta Estado de Cuenta - Contabilizado"
End Sub

Sub ConsultarContabilizadoCompleto()
    ' Ejecuta la consulta sin filtros (todos los registros CONTABILIZADO)
    
    Call ConsultarEstadoCuentaContabilizado("", "", "", "")
    
End Sub

Sub ConsultarContabilizadoPorEmpleado()
    ' Ejecuta la consulta filtrando por cedula de empleado
    
    Dim cedula As String
    
    cedula = InputBox("Ingrese la cedula del empleado:", "Consulta Estado Contabilizado")
    ...
    Call ConsultarEstadoCuentaContabilizado(cedula, "", "", "")
    
End Sub

Sub ConsultarContabilizadoPorFecha()
    ' Ejecuta la consulta filtrando por rango de fechas
    
    Dim FechaDesde As String
    Dim FechaHasta As String
    
    FechaDesde = InputBox("Ingrese la fecha inicial (formato: YYYY-MM-DD):", "Consulta Estado Contabilizado", "2026-01-01")
    ...
    FechaHasta = InputBox("Ingrese la fecha final (formato: YYYY-MM-DD):", "Consulta Estado Contabilizado", "2026-12-31")
    ...
    Call ConsultarEstadoCuentaContabilizado("", "", FechaDesde, FechaHasta)
