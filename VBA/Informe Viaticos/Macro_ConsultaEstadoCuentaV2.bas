'Attribute VB_Name = "ModuloConsultaEstadoCuentaV2"
' Macro para consultar estado de cuenta de viaticos V2 (Detallado por Estados) desde PostgreSQL
' Base de datos: solid
...
Sub ConsultarEstadoCuentaViaticosV2(Optional v_empleado As String = "", _
                                    Optional v_nombreempleado As String = "", _
                                    Optional v_desde As String = "", _
                                    Optional v_hasta As String = "")
    ' Conecta a PostgreSQL y consulta el estado de cuenta de viaticos V2 (Detallado por Estados)
    ' Parametros opcionales:
    '   v_empleado: Cedula del empleado (ej: '1105613195')
    '   v_nombreempleado: Nombre del empleado (ej: 'gladys' o 'gladys amparo')
    '   v_desde: Fecha desde (formato: 'YYYY-MM-DD', ej: '2026-01-01')
    '   v_hasta: Fecha hasta (formato: 'YYYY-MM-DD', ej: '2026-12-31')
    ' Los resultados se muestran en una hoja de Excel
    ' Desglosa los valores en 6 columnas segun el estado (CONTABILIZADO, EN FIRME, PENDIENTE)
    ...
    ' Parte 2: CTE movimientos (UNION ALL de consignacion y legalizacion)
    Dim sqlMovConsignacion As String
    Dim sqlMovUnion As String
    Dim sqlMovLegalizacion As String
    
    sqlMovConsignacion = "movimientos AS (" & vbCrLf & _
                         "    SELECT " & vbCrLf & _
                         "        codigo, " & vbCrLf & _
                         "        fechaaplicacion, " & vbCrLf & _
                         "        empleado, " & vbCrLf & _
                         "        nombreempleado, " & vbCrLf & _
                         "        codigoconsignacion AS radicado, " & vbCrLf & _
                         "        valor::numeric AS valor, " & vbCrLf & _
                         "        'CONSIGNACION' AS tipo, " & vbCrLf & _
                         "        UPPER(TRIM(estado)) AS estado_limpio, " & vbCrLf & _
                         "        observaciones " & vbCrLf & _
                         "    FROM consignacion" & vbCrLf
    
    sqlMovUnion = "    " & vbCrLf & _
                  "    UNION ALL" & vbCrLf & _
                  "    " & vbCrLf
    
    sqlMovLegalizacion = "    SELECT " & vbCrLf & _
                         "        codigo, " & vbCrLf & _
                         "        fechaaplicacion, " & vbCrLf & _
                         "        empleado, " & vbCrLf & _
                         "        nombreempleado, " & vbCrLf & _
                         "        codigolegalizacion AS radicado, " & vbCrLf & _
                         "        valortotal::numeric AS valor, " & vbCrLf & _
                         "        'LEGALIZACION' AS tipo, " & vbCrLf & _
                         "        UPPER(TRIM(estado)) AS estado_limpio, " & vbCrLf & _
                         "        observaciones " & vbCrLf & _
                         "    FROM legalizacion" & vbCrLf & _
                         ")" & vbCrLf
    
    sqlMovimientos = sqlMovConsignacion & sqlMovUnion & sqlMovLegalizacion
    
    ' Parte 3: SELECT inicial (columnas basicas)
    sqlSelect1 = "SELECT " & vbCrLf & _
                 "    m.codigo AS ""CODIGO""," & vbCrLf & _
                 "    m.fechaaplicacion::timestamp AS ""FECHA APLICACION""," & vbCrLf & _
                 "    m.empleado AS ""CEDULA""," & vbCrLf & _
                 "    m.nombreempleado AS ""EMPLEADO""," & vbCrLf & _
                 "    m.radicado AS ""RADICADO""," & vbCrLf
    
    ' Parte 4: SELECT valores por estado (CONTABILIZADO, EN FIRME, PENDIENTE)
    Dim sqlSelect2Contab As String
    Dim sqlSelect2Firmadas As String
    Dim sqlSelect2Pendientes As String
    
    sqlSelect2Contab = "    " & vbCrLf & _
                       "    -- VALORES CONTABILIZADOS" & vbCrLf & _
                       "    CASE WHEN m.tipo = 'CONSIGNACION' AND m.estado_limpio = 'CONTABILIZADO' THEN m.valor ELSE 0 END AS ""VALOR CONSIGNACION CONTABILIZADO""," & vbCrLf & _
                       "    CASE WHEN m.tipo = 'LEGALIZACION' AND m.estado_limpio = 'CONTABILIZADO' THEN m.valor ELSE 0 END AS ""VALOR LEGALIZACION CONTABILIZADO""," & vbCrLf
    
    sqlSelect2Firmadas = "    " & vbCrLf & _
                         "    -- VALORES EN FIRME (FIRMADAS)" & vbCrLf & _
                         "    CASE WHEN m.tipo = 'CONSIGNACION' AND m.estado_limpio = 'EN FIRME' THEN m.valor ELSE 0 END AS ""VALOR CONSIGNACION FIRMADAS""," & vbCrLf & _
                         "    CASE WHEN m.tipo = 'LEGALIZACION' AND m.estado_limpio = 'EN FIRME' THEN m.valor ELSE 0 END AS ""VALOR LEGALIZACION FIRMADAS""," & vbCrLf
    
    sqlSelect2Pendientes = "    " & vbCrLf & _
                           "    -- VALORES PENDIENTES" & vbCrLf & _
                           "    CASE WHEN m.tipo = 'CONSIGNACION' AND m.estado_limpio = 'PENDIENTE' THEN m.valor ELSE 0 END AS ""VALOR CONSIGNACION PENDIENTES""," & vbCrLf & _
                           "    CASE WHEN m.tipo = 'LEGALIZACION' AND m.estado_limpio = 'PENDIENTE' THEN m.valor ELSE 0 END AS ""VALOR LEGALIZACION PENDIENTES""," & vbCrLf
    
    sqlSelect2 = sqlSelect2Contab & sqlSelect2Firmadas & sqlSelect2Pendientes
    
    ' Parte 5: SELECT saldo acumulado y observaciones
    Dim sqlSelect3Saldo As String
    Dim sqlSelect3Obs As String
    
    sqlSelect3Saldo = "    " & vbCrLf & _
                       "    -- SALDO ACUMULADO" & vbCrLf & _
                       "    SUM(" & vbCrLf & _
                       "        CASE " & vbCrLf & _
                       "            WHEN m.tipo = 'CONSIGNACION' THEN m.valor" & vbCrLf & _
                       "            WHEN m.tipo = 'LEGALIZACION' THEN -m.valor" & vbCrLf & _
                       "            ELSE 0" & vbCrLf & _
                       "        END" & vbCrLf & _
                       "    ) OVER (" & vbCrLf & _
                       "        PARTITION BY m.empleado" & vbCrLf & _
                       "        ORDER BY m.fechaaplicacion::timestamp ASC, m.codigo ASC" & vbCrLf & _
                       "    ) AS ""SALDO""," & vbCrLf
    
    sqlSelect3Obs = "    " & vbCrLf & _
                    "    m.observaciones AS ""OBSERVACIONES""" & vbCrLf
    
    sqlSelect3 = sqlSelect3Saldo & sqlSelect3Obs
    
    ' Parte 6: FROM y JOINs
    sqlFrom = "FROM movimientos m" & vbCrLf & _
              "CROSS JOIN params p" & vbCrLf
    
    ' Parte 7: WHERE inicial
    Dim sqlWhere1Part1 As String
    Dim sqlWhere1Part2 As String
    
    sqlWhere1Part1 = "WHERE 1=1" & vbCrLf & _
                     "  AND (p.v_empleado IS NULL OR m.empleado = p.v_empleado)" & vbCrLf & _
                     "  AND (" & vbCrLf & _
                     "        p.v_nombreempleado IS NULL" & vbCrLf
    
    sqlWhere1Part2 = "        OR m.nombreempleado ILIKE ('%' || p.v_nombreempleado || '%')" & vbCrLf & _
                     "      )" & vbCrLf
    
    sqlWhere1 = sqlWhere1Part1 & sqlWhere1Part2
    
    ' Parte 8: WHERE fechas
    sqlWhere2 = "  AND m.fechaaplicacion::timestamp >= COALESCE(p.v_desde::timestamp, '1900-01-01'::timestamp)" & vbCrLf & _
                "  AND m.fechaaplicacion::timestamp < (COALESCE(p.v_hasta::timestamp, '9999-12-31'::timestamp) + INTERVAL '1 day')" & vbCrLf
    
    ' Parte 9: ORDER BY
    sqlOrder = "ORDER BY m.empleado, m.fechaaplicacion::timestamp ASC, m.codigo ASC"
    
    ' Construir consulta completa usando concatenacion simple
    consultaSQL = sqlWith & sqlMovimientos & sqlSelect1 & sqlSelect2 & sqlSelect3 & sqlFrom & sqlWhere1 & sqlWhere2 & sqlOrder
    ...
    Debug.Print "=========================================="
    Debug.Print "DIAGNOSTICO DE RENDIMIENTO - ESTADO CUENTA V2"
    Debug.Print "=========================================="
    ...
    Debug.Print "Ejecutando consulta de estado de cuenta V2..."
    ...
    Set ws = ThisWorkbook.Worksheets("Estado_Cuenta_V2")
    If ws Is Nothing Then
        Set ws = ThisWorkbook.Worksheets.Add
        ws.Name = "Estado_Cuenta_V2"
    Else
    ...
    On Error Resume Next
    ws.ListObjects.Add(xlSrcRange, rangoTabla, , xlYes).Name = "TablaEstadoCuentaV2"
    On Error GoTo ErrorHandler
    
    ' Formatear tabla de Excel
    With ws.ListObjects("TablaEstadoCuentaV2")
    ...
    ' Formatear columnas numericas (valores monetarios)
    On Error Resume Next
    ' Formatear las columnas de valores como moneda (columnas F:K y L)
    ' F: VALOR CONSIGNACION CONTABILIZADO
    ' G: VALOR LEGALIZACION CONTABILIZADO
    ' H: VALOR CONSIGNACION FIRMADAS
    ' I: VALOR LEGALIZACION FIRMADAS
    ' J: VALOR CONSIGNACION PENDIENTES
    ' K: VALOR LEGALIZACION PENDIENTES
    ' L: SALDO
    ws.Columns("F:K").NumberFormat = "$#,##0;[Red]-$#,##0"
    ws.Columns("L").NumberFormat = "$#,##0;[Red]-$#,##0"
    ws.Columns("B").NumberFormat = "dd/mm/yyyy"
    ' Centrar el contenido de la columna B (fecha)
    ...
    ws.Columns("A").ColumnWidth = 15 ' CODIGO
    ws.Columns("B").ColumnWidth = 15 ' FECHA APLICACION
    ws.Columns("C").ColumnWidth = 15 ' CEDULA
    ws.Columns("D").ColumnWidth = 40 ' EMPLEADO
    ws.Columns("E").ColumnWidth = 15 ' RADICADO
    ws.Columns("F:K").ColumnWidth = 20 ' Valores monetarios
    ws.Columns("L").ColumnWidth = 20 ' SALDO
    ws.Columns("M").ColumnWidth = 100 ' OBSERVACIONES

    ws.Range("A7:" & ultimaColumna & "7").WrapText = True
    ws.Range("A7:" & ultimaColumna & "7").VerticalAlignment = xlCenter
    
    ' Agregar formulas de totales en la fila 6
    ws.Range("F6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR CONSIGNACION CONTABILIZADO])"
    ws.Range("G6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR LEGALIZACION CONTABILIZADO])"
    ws.Range("H6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR CONSIGNACION FIRMADAS])"
    ws.Range("I6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR LEGALIZACION FIRMADAS])"
    ws.Range("J6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR CONSIGNACION PENDIENTES])"
    ws.Range("K6").FormulaR1C1 = "=+SUBTOTAL(9,TablaEstadoCuentaV2[VALOR LEGALIZACION PENDIENTES])"
    ws.Range("F6:K6").NumberFormat = "$#,##0;[Red]-$#,##0"
    ws.Range("F6:K6").Font.Bold = True
    ws.Range("F5").Value = "TOTALES"
    ...
    mensajeExito = "Consulta completada exitosamente." & vbCrLf & vbCrLf & _
                   ...
                   "Los resultados se encuentran en la hoja 'Estado_Cuenta_V2'" & vbCrLf & _
                   "Ver ventana Inmediato (Ctrl+G) para mas detalles."
    
    MsgBox mensajeExito, vbInformation, "Consulta Estado de Cuenta V2"
...
ErrorHandler:
    ...
    Application.StatusBar = False
    Application.ScreenUpdating = True
    MsgBox mensajeError, vbCritical, "Error en Consulta Estado de Cuenta V2"
End Sub

Sub ConsultarEstadoCuentaV2Completo()
    ' Ejecuta la consulta sin filtros (todos los registros)
    ' Util para obtener un reporte completo
    
    Call ConsultarEstadoCuentaViaticosV2("", "", "", "")
    
End Sub

Sub ConsultarEstadoCuentaV2PorEmpleado()
    ' Ejecuta la consulta filtrando por cedula de empleado
    ' Solicita la cedula mediante un InputBox
    
    Dim cedula As String
    
    cedula = InputBox("Ingrese la cedula del empleado:", "Consulta Estado de Cuenta V2")
    ...
    Call ConsultarEstadoCuentaViaticosV2(cedula, "", "", "")
    
End Sub

Sub ConsultarEstadoCuentaV2PorFecha()
    ...
    FechaDesde = InputBox("Ingrese la fecha inicial (formato: YYYY-MM-DD):", "Consulta Estado de Cuenta V2", "2026-01-01")
    ...
    FechaHasta = InputBox("Ingrese la fecha final (formato: YYYY-MM-DD):", "Consulta Estado de Cuenta V2", "2026-12-31")
    ...
    Call ConsultarEstadoCuentaViaticosV2("", "", FechaDesde, FechaHasta)
