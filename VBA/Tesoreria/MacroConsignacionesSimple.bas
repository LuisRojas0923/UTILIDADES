'Attribute VB_Name = "ModuloConsignacionesSimple"
' Macro para generar el reporte de CONSIGNACIONES (Version Simple)
' Sin columnas: IMP 4 X 1000, TOTAL CONSIGNACION, VIATICO A PAGAR?
...
Sub EjecutarConsignacionesSimple()
    Dim conn As Object
    Dim rs As Object
    Dim ws As Worksheet
    Dim sql As String
    ...
    ' 3. PREPARAR CONSULTA SQL (Sin IMP 4 X 1000, TOTAL CONSIGNACION y VIATICO A PAGAR?)
    sql = "SELECT " & vbCrLf & _
    ...
    If totalRegs >= filaEncabezado + 1 Then
        ...
        ' 2. Totales (En Fila 11, encima de encabezados) - Solo CONSIGNACION
        tbl.ShowTotals = False
        ws.Range("H11").Formula = "=SUBTOTAL(9,Consignaciones_Viaticos[CONSIGNACION])"
        
        ' Formato Moneda para el subtotal superior
        ws.Range("H11").NumberFormat = "$#,##0"
        ws.Range("H11").Font.Bold = True
        
        ' 3. Formato Moneda Columna CONSIGNACION
        ws.Columns("H:H").NumberFormat = "$#,##0" ' Columna CONSIGNACION
    ...
    MsgBox "Reporte de Consignaciones (Simple) generado exitosamente.", vbInformation
    Exit Sub
    Application.ScreenUpdating = True
ErrorHandlerSimple:
    Application.StatusBar = False
